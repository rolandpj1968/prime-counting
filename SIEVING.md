# Straight sieving: empirical findings
...
28 GiB RAM. Build: `zig build-exe -O ReleaseFast -mcpu=native`.

Everything is one generic sieve, `Sieve(Wheel, Store, seg_bytes)`:
...
| knob | finding | impact |
|---|---|---|
| **Wheel** (2,3,5) | strike-count thinned by density φ(M)/M; matches Mertens theory to 2 dp | **6.05×** all→mod-30 |
| **Segmentation** | working set pinned in L1d, cache-immune to N | ~1.5–3× vs whole-array in DRAM |
| **Segment size** | optimum = **L1d (32 KiB)**; sharp L3→DRAM cliff at 16 MiB | reads the cache hierarchy off the curve |
| **Store representation** | optimum **flips** with regime (bit vs byte) | up to ~2.4× either way |
| **Wheel depth** | peaks at **mod-30**; mod-210+ regresses (delta tables spill L2) | +6× to mod-30, then −2.2× |
| **Bucket sieve** | only helps when √N > seg_slots (large-N / small-seg) | 1.22× at N=10¹⁰/1 KiB, climbing |
| **Sieve of Atkin** | better op-count, but memory-bound at scale + can't segment | 1.8× in-cache, **moot** at scale |
| **u128 range sieve** | counts primes past 2⁶⁴, validated by PNT not tables | reaches ~10²⁰ |

The champion is **bit-packed `[]u64` + mod-30, ~3.6 G ints/s** — exactly where
production sieves sit, here understood from first principles. Details below.
...
The engineering built the engine; the engine demonstrates the theory (PNT, Li, RH).
